...
public class Model{
	...
	public void setMultiThread(boolean useIndex) throws Exception {
		...
		for(int i = 0;i < thread_num;i++){
			...
			threads[i].n_z = new int[K];
			threads[i].n_zv = new int[K][V];
			threads[i].m_z = new int[K];

//			for(int k = 0;k < K;k++){
//				threads[i].n_z[k] = n_z[k];
//				threads[i].m_z[k] = m_z[k];
//				for(int v = 0;v < V;v++){
//					threads[i].n_zv[k][v] = n_zv[k][v];
//				}
//			}
//			threads[i].n_z_change = new int[K];
//			threads[i].n_zv_change = new int[K][V];
//			threads[i].m_z_change = new int[K];
		}
	}
